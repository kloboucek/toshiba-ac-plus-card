...
- The thermostat dial supports both pointer and touch drag handling. On mobile, vertical swipes over the dial/arc and center temperature readout are left to the dashboard page for scrolling; grabbing the slider thumb locks the gesture to the dial, while areas outside the arc/thumb grab targets remain available for page scrolling. After release, the card keeps the selected target temperature visible optimistically while Home Assistant applies the service call in the background.
